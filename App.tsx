...
const App: React.FC = () => {
  ...
  const handleFileChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // FIX: Explicitly type `file` as `File` to fix type inference issues where it was being treated as `unknown`.
  ...
  const handleReorderThumbnails = useCallback((dragIndex: number, hoverIndex: number) => {
    setThumbnails(prev => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 text-zinc-900 font-sans">
      <header className="bg-white/80 backdrop-blur-sm sticky top-0 z-10 p-4 shadow-md border-b border-zinc-200">
      ...
      <main className="container mx-auto p-4 md:p-8">
        ...
        {thumbnails.length === 0 ? (
          <div className="text-center py-20">
            <div
              onClick={triggerFileSelect}
              className="cursor-pointer group inline-block p-10 border-2 border-dashed border-zinc-300 hover:border-blue-500 rounded-2xl transition-all duration-300 hover:bg-gray-200"
            >
          ...
          <ThumbnailGrid
            ...
            onReorder={handleReorderThumbnails}
          />
        )}
      </main>
    </div>
